...
Cancel the preserved pg-boss job IDs before restarting. This must run while the new release code is checked out:
...
(async () => {
  ...
  for (const campaign of rows) {
    await scheduler.requestStop(campaign.id, campaign.tenant_id);
  }
